...
{
  /*
   * FALCON: buffers are static arrays of TWI_BUFFER_SIZE now, not `new`. See
   * Wire.h. Clamp bufferLength so a caller asking for more than the compiled
   * size cannot walk off the end -- upstream would have honoured the request by
   * allocating, and every bounds check downstream tests against bufferLength.
   */
  if (bufferLength > (int)TWI_BUFFER_SIZE) {
    bufferLength = (int)TWI_BUFFER_SIZE;
  }
...
{
  /*
   * FALCON: nothing to free -- the buffers are static arrays now (Wire.h), and
   * `delete[]` on them would be undefined behaviour. This destructor is also
   * unreachable in practice: the only instance is the global Wire1, which lives
...
/*
 * ─── FALCON LOCAL MODIFICATION: `Wire` (TWI0) IS NOT INSTANTIATED ────────────
 *
 * NOTHING IS ON TWI0. The DPS310 that used to sit there is depopulated and the
 * pressure path was deleted in cfe0b56, so no source file in this project
 * references `Wire` -- only `Wire1`, which carries the BMA456.
 *
 * But upstream preinstantiates `TwoWire Wire` here, and that constructor takes
 * the whole TWI0 driver as FUNCTION POINTERS. --gc-sections therefore cannot
 * prove any of it dead, so an entire I2C bus with nothing connected to it was
 * being linked into a firmware with 30 bytes free:
 *
 *     __vector_24 (TWI0 ISR)                             600
 *     twi_writeTo / twi_readFrom / twi_transmit          366
 *     twi_init / twi_disable                              78
 *     twi_setFrequency / setAddress / reply / stop / …     94
 *     ------------------------------------------------  ----
 *                                                       ~1138 bytes of flash
 *
 * plus 64 bytes of RAM in TWI0's own twi_rxBuffer/twi_txBuffer and two heap
 * allocations for this object's buffers.
 *
 * Deleting this one definition makes all of it unreferenced, which is why
 * utility/twi.c is deliberately ABSENT from this vendored copy -- see
 * lib/Wire1 for the sibling modification and the reason vendoring works
 * (project libs take precedence over framework ones; verified in the
 * dependency graph, and the unmodified copies build byte-identical).
 *
 * ⚠️ IF ANYTHING IS EVER FITTED TO TWI0: restore this block AND copy
 * utility/twi.c back in. The link will fail loudly rather than misbehave,
 * because `Wire` simply will not exist.
 *
 * The TwoWire *methods* above are NOT removed -- Wire1 is an instance of this
 * same class and needs every one of them.
 */
