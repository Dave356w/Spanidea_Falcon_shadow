...
/**
 * check and move to next state. setup the initial conf for the new state.
 */
void MovementService::next()
{
    timer_ms = millis();
    acc_varience_counter = 0;
    pressure_varience_counter = 0;
...
void MovementService::monitor()
{
    // Perform the following check regardless of whether monitoring mode is enabled or not
    ...
    // do not execute more if monitoring mode is not enabled
    if (monitor_state == MonitorStates::NOT_MONITORING) {
        return ;
    }
    ...
    case MotionStates::MOVEMENT_DETECTED:
        /* 
         * Lets wait for few ms for device to accelerate little bit and 
         * calculate/decide the velocity threshold
         */
