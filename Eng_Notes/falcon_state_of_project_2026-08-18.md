...
| Constant | Value | Basis | Status |
|---|---|---|---|
| `ARRIVAL_PEAK_VALUE` | 0.45 | geometric middle of cruise 0.28 / arrival 0.713 | 🔴🔴 **RELEASED THE BEACON ON A MOVING CAR, 2026-08-20** — a cruise transient of 0.472 mid-travel, eleven seconds of silence, car confirmed moving by lateral 0.2–0.884. Real arrivals on the same build ran 0.450–0.511, so **the false release sits INSIDE the real-arrival population and no threshold separates them.** `falcon_false_release_2026-08-20.md` |
...
| `ARRIVAL_ARM_SAMPLES` | 5 | 200 ms at 25 Hz | 🟢 **`arm_via=2` FIRED — the reversal path armed 2 of 32 runs on 2026-08-20 with `q`=4, i.e. exactly when quiet could not, and once before on 2026-08-13. All three clean releases. "Never fired" was stale.** 🔴 **`q` hit exactly 5 on 8 of 32 contract-speed runs (2026-08-20), and on run 10 the reversal path was simultaneously unavailable (`g=0 ro=7`) — arming rested on one sample of one path with no backup.** Cruise on that mounting never advances the counter at all (lateral 0.134–0.649 vs a 0.06 still threshold), so all five samples come from post-stop ringdown. `falcon_automatic_2026-08-20.md` §2, §4 |
